...
def gameLoop():
    ...
    while not game_over:
        ...
        while game_close == True:
            dis.fill(blue)
            message("你输了! 按Q退出或按C重玩", red)
            Your_score(Length_of_snake - 1)
            pygame.display.update()

            # 绘制按钮
            quit_button = pygame.Rect(50, 200, 100, 50)  # 退出按钮的位置和大小
            continue_button = pygame.Rect(200, 200, 100, 50)  # 继续按钮的位置和大小

            pygame.draw.rect(dis, red, quit_button)
            pygame.draw.rect(dis, green, continue_button)

            # 绘制按钮文本
            dis.blit(font_style.render("退出", True, white), (60, 210))
            dis.blit(font_style.render("重玩", True, white), (210, 210))
